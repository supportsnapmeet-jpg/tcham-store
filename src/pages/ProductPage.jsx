...
import { ArrowLeft, ShoppingBag, MessageCircle, Heart, ChevronRight } from 'lucide-react'
...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    getProductById(id)
      ...
      .then((all) => {
        const rel = all.filter((p) => p.category === product?.category && p.id !== id)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-14">
        {/* Images */}
        <div>
          <div className="relative aspect-square bg-stone-50 flex items-center justify-center overflow-hidden mb-2">
            ...
            {product.badge && (
              <span className="absolute top-4 left-4 bg-gray-900 text-white text-xs font-bold tracking-widest px-3 py-1.5">{product.badge}</span>
            )}
            <button onClick={() => setLiked(!liked)} className="absolute top-4 right-4 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow">
            ...
            <div className="flex gap-2">
              {product.images.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)}
                  className={`w-16 h-16 border-2 overflow-hidden transition-colors ${activeImg === i ? 'border-gray-900' : 'border-transparent'}`}>
        ...
        <div className="flex flex-col">
          ...
          {product.colors?.length > 0 && (
            <div className="mb-5">
              <p className="text-xs font-bold tracking-widest uppercase text-gray-700 mb-3">
                Couleur : <span className="text-gold-600">{selectedColor}</span>
              </p>
          ...
          {product.sizes?.length > 0 && product.sizes[0] !== 'Taille unique' && (
            <div className="mb-6">
              <p className="text-xs font-bold tracking-widest uppercase text-gray-700 mb-3">
                Taille : <span className="text-gold-600">{selectedSize}</span>
              </p>
              <div className="flex flex-wrap gap-2">
                {product.sizes.map((s) => (
          ...
          <div className="mt-6 pt-6 border-t border-gray-100 space-y-2 text-xs text-gray-400">
            ...
            <p>💳 Paiement : Wave · Orange Money · MTN MoMo</p>
